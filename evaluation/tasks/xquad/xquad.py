# HuggingFace dataset link: https://huggingface.co/datasets/xquad
import re
import string
...
class XQuADDataset(Dataset):
    def __init__(self, tokenizer, target_langs):
        ...
        for target_lang in target_langs:
            xquad_lang = load_dataset("xquad", lang2config[target_lang], split="validation")
            for sample in xquad_lang:
...
# Evaluation of F1 and EM from the official SQuAD evaluate-v1.1.py script (https://github.com/deepmind/xquad)
...
class XQuADTask(AutoTask):
    @staticmethod
    def get_display_name() -> str:
        return "xquad"

    def evaluate(self) -> None:
        dataset = XQuADDataset(self.tokenizer, target_langs=self.task_config["target_langs"])
