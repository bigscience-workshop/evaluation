...
class EvaluationArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "The model checkpoint that we want to evaluate, could be name or the path."
        },
    )
    ...
    output_dir: str = field(
        default="outputs", metadata={"help": "Directory for saving evaluation outputs."}
    )
    tag: Optional[str] = field(
        default=None, metadata={"help": "Identifier for the evaluation run."}
    )
    random_seed: int = field(default=24, metadata={"help": "Customized random seed"})
    eval_tasks: Optional[List[str]] = field(
        default=None,
        metadata={
            "help": "A list of tasks to run the evaluation on, e.g. tydiqa_secondary"
        },
    )
    device: str = field(
        default="cuda", metadata={"help": "Device on which to run evaluation"}
    )
...
def main(args):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.model_name_or_path, pad_token_id=tokenizer.eos_token
    )
    ...
    for eval_task in args.eval_tasks:
        ...
        task = AutoTask.from_task_name(
            eval_task, tokenizer=tokenizer, model=model, device=device
        )
